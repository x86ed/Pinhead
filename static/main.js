...
myHeaders.append(
  'Authorization',
  'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdXRob3JpemVkIjp0cnVlLCJleHAiOjE2NTU2Njg5MjAsIm5hbWUiOiJkZXJwIiwicm9sZSI6InVzZXIifQ.w09Rwoa7X0Fu5aMXrvQ5KMwA5VeSpMhSJ1j24snTdJU'
);
      ...
      if (com.dom_id.length) {
        if (com.down_command.length) {
          if (!com.up_command.length) {
            document.getElementById(com.dom_id).onclick = () => {
              wsSend(com.down_command);
            };
          } else {
            document.getElementById(com.dom_id).onmousedown = () => {
              wsSend(com.down_command);
            };
          }
        }
        if (com.up_command.length) {
          document.getElementById(com.dom_id).onmouseup = () => {
